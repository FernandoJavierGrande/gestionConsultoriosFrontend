...
import { Home, Users, Stethoscope, Calendar, BriefcaseMedical, Hospital, LogOut, NotebookText, Shield } from "lucide-react";

function Navbar({ onLogout, usuarioActual }) {
      ...
      <div className="mx-auto px-4 w-full">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-1">
            ...
            {/* Usuario logueado */}
            <div className="flex items-center gap-2 px-4 py-2 ml-2 border-l border-blue-500 text-blue-100 text-sm">
              <span>{usuarioActual?.user}</span>
              <span className={`text-xs px-2 py-0.5 rounded-full ${
                usuarioActual?.permisos === "admin" ? "bg-purple-500" : "bg-blue-500"
              }`}>
            ...
            <button
              onClick={onLogout}
              className="flex items-center gap-2 px-4 py-2 rounded-lg transition-all duration-200 hover:bg-red-500 hover:bg-opacity-50"
              title="Cerrar Sesión"
            >
              <LogOut size={18} />
              <span>Salir</span>
